...
* В задании №4 предполагаемая реализация не подходила по требованиям по памяти - массив counts_ слишком большой(n^2).
* В случае последнего текста лишь 6% от всех элементов ненулевые, поэтому более оптимальным решением(которое сейчас реализовано) оказалось использование std::map<std::pair<size_t, size_t>, size_t> вместо двумерного массива.
* По итогу формат задач мне кажется интересный
